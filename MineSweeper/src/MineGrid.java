...
class MineGrid {
	...
	public boolean isInsideGrid(int rowIndex, int columnIndex) {
		return (rowIndex >= 0 && rowIndex < this.mineInformation.length)
				&& (columnIndex >= 0 && columnIndex < this.mineInformation[0].length);
	}
	...
	public void setCellContent(int row, int col, String text) {
		this.buttons[row][col].setText(text);
	}
}
